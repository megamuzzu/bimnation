...
class BaseController extends CI_Controller {
	...
	function isAgentLoggedIn() {
		...
		$isLoggedIn = $this->session->userdata ( 'isAgentLoggedIn' );
		$agentId = $this->session->userdata ( 'agentId' );
		if (! isset ( $isLoggedIn ) || $isLoggedIn != TRUE || $agentId =='') {
			...
			$where = array();
			 ...
			 $result = $this->staff_model->findDynamic($where);


			 	if(!empty($result))
			 	{
			 		$this->session->set_userdata (array('role'=>'agent','userId'=>$agentId,'name'=>$result[0]->StaffName) );
			 	}
    ...
    function agentView($viewName = "", $headerInfo = NULL, $pageInfo = NULL, $footerInfo = NULL){



        $this->load->view('agent/includes/header', $headerInfo);

        $this->load->view($viewName, $pageInfo);

        $this->load->view('agent/includes/footer', $footerInfo);
     ...
     function userVendorView($viewName = "", $headerInfo = NULL, $pageInfo = NULL, $footerInfo = NULL){
        ...
        $this->load->view('userVendor/includes/header', $headerInfo);

        $this->load->view($viewName, $pageInfo);

        $this->load->view('userVendor/includes/footer', $footerInfo);
